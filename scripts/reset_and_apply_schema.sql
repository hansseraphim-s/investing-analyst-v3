-- ============================================================================
-- investing-analyst-v3 — RESET + APPLY schema (one-shot)
--
-- Drops any pre-existing iav3 tables in the public schema, then recreates
-- them from scratch matching shared/schema.sql. Use this when:
--   - The Neon database had leftover tables from a prior project
--   - You want a known-clean baseline for paper trading
--
-- DESTRUCTIVE — wipes all data in these tables. Safe to run on a fresh
-- Neon database that's never written to. Do NOT run against a database
-- that has data you care about.
-- ============================================================================

-- Drop in reverse-dependency order. CASCADE handles any FKs we forgot.
DROP TABLE IF EXISTS kill_switch_events CASCADE;
DROP TABLE IF EXISTS walk_forward_runs  CASCADE;
DROP TABLE IF EXISTS greeks_snapshot    CASCADE;
...
CREATE INDEX idx_equity_curve_recorded_at ON equity_curve (recorded_at DESC);
...
CREATE INDEX idx_greeks_snapshot_at ON greeks_snapshot (snapshot_at DESC);
...
CREATE INDEX idx_walk_forward_run_id ON walk_forward_runs (run_id);
...
-- Quick verification — should return 8 rows.
SELECT table_name FROM information_schema.tables
WHERE table_schema = 'public'
  AND table_name IN ('sessions', 'orders', 'positions', 'equity_curve',
                     'signals', 'greeks_snapshot', 'walk_forward_runs',
                     'kill_switch_events')
ORDER BY table_name;
